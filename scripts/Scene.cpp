...
int Scene::Update(float deltaTime) {
	if (isGame) {
        ...
        for (size_t i = 0; i < state.enemies.size(); i++){
            state.enemies[i]->Update(deltaTime, state.map, state.enemies, state.player);
        }
        
        //Update Player Bullets
        for (size_t i = 0; i < state.playerBullets.size(); i++){
            state.playerBullets[i]->Update(deltaTime, state.map, state.enemies, state.player);
        }
...
void Scene::Render(ShaderProgram *program, const string& gameState) {
	...
	if (isGame) {
        ...
        for (Entity* bullet : state.playerBullets){
...
void Scene::makePlayerBullet(){
    Bullet* tempBullet = new Bullet(LoadTexture("./src/bullet.png"),1.0f, 1.0f);
    tempBullet->position = state.player->position;
    tempBullet->movement = -(state.player->movement);
    state.playerBullets.push_back(tempBullet);
}
